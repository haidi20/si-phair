...
class BargeController extends Controller
{
    public function index(Datatables $datatables)
    {
        ...
        if ($datatables->getRequest()->ajax()) {
            $barge = Barge::query()
                ->select('barges.id', 'barges.name', 'barges.description');

            return $datatables->eloquent($barge)
                ...
                ->filterColumn('description', function (Builder $query, $keyword) {
                    $sql = "barges.description like ?";
                    $query->whereRaw($sql, ["%{$keyword}%"]);
                })
                ->addColumn('aksi', function (Barge $data) {
                    $button = '';
        ...
        $compact = compact('html', 'barges');

        return view("pages.master.barge.index", $compact);
    }
