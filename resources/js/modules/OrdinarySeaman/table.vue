...
export default {
  ...
  computed: {
    ...
    getLoadingTable() {
      return this.$store.state.os.loading.table;
    },
    getIsShowForm() {
      return this.$store.state.os.form.is_show_form;
    },
  },
  methods: {
    onEdit(form) {
      ...
      this.$store.commit("os/INSERT_FORM", {
        form: form,
      });
    },
    async onDelete(data) {
      ...
      await Swal.fire({
        ...
        if (result.isConfirmed) {
          await axios
            .post(`${this.getBaseUrl}/api/v1/ordinary-seaman/delete`, {
              id: data.id,
