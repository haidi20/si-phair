...
<table>
    ...
    <tr>
        ...
        <td>Sisa Cuti</td>
        <td><br></td>
        <td colspan="2">{{$employee->remaining_time_off ?? ''}}</td>
        <td>hari/tahun</td>
    </tr>
    ...
    <tr>
        ...
        <td>Jam Kembali Istirahat</td>

        <td>Jam Pulang</td>
        <td>Jam Mulai Lembur</td>
        <td>Jam Pulang Lembur</td>
    ...
    if($a->hour_start == null && $a->hour_end == null){
        $a->duration_work = null;
    }else{
        if($a->hour_overtime_end != null && $a->hour_start != null){
        $a->duration_work = $a->jam_kerja_lembur_kotor;
    }else{
        $a->duration_work = $a->jam_kerja_kotor;
    }
    }
    ...
    $jam_kerja_kotor_hour = \floor((($a->duration_work))/60);
    $jam_kerja_kotor_minute = ((($a->duration_work))/60)%60;


    $jam_kerja_bersih_hour = \floor((($a->duration_work) - $a->duration_rest)/60);
    $jam_kerja_bersih_minute = ((($a->duration_work) - $a->duration_rest)/60)%60;
...
@endphp
        <tr style="background-color:red">
            ...
            {{--  --}}
            <td>{{$a->jam_masuk_real}}</td>
            
            <td>{{$a->jam_istirahat_keluar_real}}</td>
            <td>{{$a->jam_istirahat_masuk_real}}</td>
            <td>{{$a->jam_pulang_real}}</td>


            <td>{{$a->jam_mulai_lembur}}</td>
            <td>{{$a->jam_pulang_lembur}}</td>

            @if($a->jam_pulang_lembur != '')
                <td>{{$a->is_nonstop == 1 ?  'Break' : 'NonStop'}}</td>
            @else
            <td><br></td>
            @endif
...
@endif
    ...
    <tr>
        ...
        <td><br></td>
        <td><br></td>
        <td data-format="Rp* #,##" colspan="2">{{\round($payroll->pemotongan_tidak_hadir)}}</td>
    ...
    <tr>
        ...
        <td><br></td>
        <td><br></td>
        <td data-format="Rp* #,##" colspan="2">{{\round($payroll->jumlah_hutang)}}</td>
    ...
    <tr>
        ...
        <td><br></td>
        <td><br></td>
        <td data-format="Rp* #,##" colspan="2">{{\round(($payroll->pemotongan_potongan_lain_lain -$payroll->jumlah_hutang))}}</td>
    ...
    <tr>
        ...
        <td><br></td>
        <td><br></td>
        <td colspan="2" data-format="Rp* #,##">{{($payroll->pajak_biaya_jabatan/12)}}</td>
        <td><br></td>
        <td><br></td>
    ...
    <tr>
        ...
        <td><br></td>
        <td><br></td>
        <td colspan="2" data-format="Rp* #,##">{{($payroll->pajak_bpjs_dibayar_karyawan/12)}}</td>
    ...
    <tr>
        ...
        <td><br></td>
        <td><br></td>
        <td colspan="2" data-format="Rp* #,##">{{($payroll->pajak_total_pengurang/12)}}</td>
        <td><br></td>
        <td><br></td>
    </tr>
    <tr>
    ...
    @php
    $year_x = \Carbon\Carbon::parse($period_payroll->period)->translatedFormat('Y');
    $month_x = \Carbon\Carbon::parse($period_payroll->period)->translatedFormat('m');    

    $detail_tambahan = \App\Models\salaryAdjustmentDetail::where('amount','>',0)->where('employee_id',$employee->id)
    ->whereNotNull('month_start')
    ->whereMonth('month_start',$month_x)
    ->whereYear('month_start',$year_x)
    ->get();
    ...
    @endphp
        ...
        @foreach ($detail_tambahan as $t)
        <tr>
            <td><br></td>
            <td><br></td>
            <td>{{$t->SalaryAdjustment->name}}</td>

            @if ($t->type_incentive == 'deduction')
            <td>Pengurang</td>
            @else
            <td>Penambahan</td>
            @endif

            <td>{{$t->SalaryAdjustment->note}}</td>

            @if ($t->type_amount == 'percent')
            <td data-format="Rp* #,##" >{{round($t->amount * $employee->basic_salary * 0.01)}}</td>
            @else
            <td data-format="Rp* #,##" >{{$t->amount}}</td>
            @endif
